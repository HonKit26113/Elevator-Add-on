 ...
 * @returns {object}
 */
export function getElevatorById(targetId) {
...
function setFloorLevel(elevatorId, floorIndex, newLevel) {
    const elevator = getElevatorById(elevatorId);
    if (!elevator)
        return console.error(`SetFloorLevel: Elevator ID ${elevatorId} not found.`);
    const floor = elevator.floors[floorIndex];
    if (!floor)
        return console.error(`SetFloorLevel: Floor index ${floorIndex} out of bounds.`);
...
export function getElevatorIdFromSelection(selection, actionNumber) {
    ...
    if (actionNumber === 1)
        elevatorButtonOffset += 1; // "Create New" button
    if (actionNumber === 3)
        elevatorButtonOffset += 1; // "Unbind Wrench" button
...
export function permissionSettings(player) {
    ...
    permissionMenu.show(player).then((response) => {
...
export function openFloorsList(player, elevatorId, block, actionNumber) {
    ...
    submenu.show(player).then(async (response) => {
        if (response.selection === undefined)
            return;
...
function editFloorProperties(player, elevatorId, floor, block) {
    ...
    floor_details.show(player).then((floor_response) => {
        if (floor_response.formValues === undefined)
            return;
...
function editElevatorProperties(player, elevatorId) {
    ...
    elevatorPropertiesPanel.show(player).then((response) => {
